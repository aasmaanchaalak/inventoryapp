...
const connectDB = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/inventoryapp');
...
const analyzeCurrentData = async () => {
  ...
  try {
    ...
    productFieldStats.forEach(stat => {
      ...
      const status = isLegacy ? '🔄 (needs migration)' : stat._id ? '✅ (current)' : '❓ (null/undefined)';
      console.log(`  ${stat._id || 'null/undefined'}: ${stat.count} leads ${status}`);
    });
      ...
      if (leadsToUpdate.length > 0) {
        ...
        if (!dryRun) {
          ...
          leadsToUpdate.slice(0, 3).forEach(lead => {
            console.log(`    - ${lead.name} (${lead.phone}) - Product: "${lead.product}"`);
          });
...
const verifyMigration = async () => {
  ...
  try {
    ...
    if (remainingLegacy.length === 0) {
      console.log('✅ Migration verification successful! No legacy categories found.');
    } else {
      console.log(`⚠️  Warning: ${remainingLegacy.length} leads still have legacy categories:`);
      remainingLegacy.forEach(lead => {
        console.log(`  - ${lead.name}: product="${lead.product}", productInterest="${lead.productInterest}"`);
      });
    }
...
const main = async () => {
  ...
  try {
    ...
    switch (action) {
      ...
      default:
        console.log('\nUsage:');
        console.log('  node migrate-product-categories.js analyze    # Analyze current data');
        console.log('  node migrate-product-categories.js migrate    # Dry run migration');
        console.log('  node migrate-product-categories.js migrate --execute  # Execute migration');
        console.log('  node migrate-product-categories.js verify     # Verify migration results');
    }
  } catch (error) {
