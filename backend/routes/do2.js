...
const getNextApprovalStep = (latestDO2, poApprovalStatus) => {
  ...
  switch (latestDO2.status) {
    case 'draft':
      return poApprovalStatus === 'approved' ? 'pending_approval' : 'wait_for_po_approval';
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        totalQuantityExecuted: do2.items.reduce((sum, item) => sum + item.remainingQuantity, 0)
      }
    });
...
router.get('/calendar', async (req, res) => {
  try {
    ...
    const calendarData = do2s.map(do2 => {
      ...
      const totalDispatchQuantity = do2.items.reduce((sum, item) => sum + item.remainingQuantity, 0);
    ...
    const summary = {
      ...
      pendingApproval: calendarData.filter(d => d.status === 'pending_approval').length,
      ...
      totalQuantity: calendarData.reduce((sum, d) => sum + d.totalDispatchQuantity, 0),
