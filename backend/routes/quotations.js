  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        message:
          'Missing required fields: leadId, items, validity, deliveryTerms, and totalAmount are required',
      });
    }
...
router.get('/:id/pdf', async (req, res) => {
  try {
    const quotation = await Quotation.findById(req.params.id).populate(
      'leadId',
      'name phone product source notes'
    );
    ...
    doc
      .fontSize(10)
      .text(
        `Phone: ${quotation.companyInfo.phone} | Email: ${quotation.companyInfo.email}`,
        { align: 'center' }
      )
      .moveDown(0.5);
    ...
    doc
      .fontSize(10)
      .text(
        `GSTIN: ${quotation.companyInfo.gstin} | PAN: ${quotation.companyInfo.pan}`,
        { align: 'center' }
      )
      .moveDown(2);
  ...
  try {
    ...
    if (!validStatuses.includes(status)) {
      return res.status(400).json({
        success: false,
        message:
          'Invalid status. Must be one of: draft, sent, accepted, rejected, expired',
      });
    }
