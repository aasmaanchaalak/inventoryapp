...
const InvoiceAuditTrail = () => {
    ...
    if (auditTrail && auditTrail.auditTrail) {
      if (selectedEventFilter === 'all') {
        ...
        const filtered = auditTrail.auditTrail.filter(
          entry => entry.event === selectedEventFilter
        );
  ...
  const getUniqueEvents = () => {
    ...
    const events = [...new Set(auditTrail.auditTrail.map(entry => entry.event))];
    return events.sort();
  };
    ...
    <div className="max-w-7xl mx-auto p-6">
      <div className="bg-white shadow-md rounded-lg">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <p className="text-gray-600 mt-1">View detailed audit logs for invoice activities</p>
        </div>
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
              <div>
                ...
                <span className="ml-2 text-gray-900">{auditTrail.invoiceNumber}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="ml-2 text-gray-900">{auditTrail.totalEvents}</span>
              </div>
              <div>
                <span className="font-medium text-gray-700">Filtered Events:</span>
                <span className="ml-2 text-gray-900">{filteredTrail.length}</span>
              </div>
            </div>
          </div>
        )}
                    ...
                    filteredTrail.map((entry, index) => (
                      <tr key={index} className="hover:bg-gray-50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full border ${getEventColor(entry.event)}`}>
