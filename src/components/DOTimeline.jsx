...
const DOTimeline = () => {
  ...
  const fetchTimelineData = async (identifier, type) => {
    ...
    try {
      let response;
      if (type === 'lead') {
        response = await fetch(`http://localhost:5000/api/leads/timeline/${identifier}`);
      } else {
        response = await fetch(`http://localhost:5000/api/pos/timeline/${identifier}`);
      }
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-md">
      ...
      <div className="mb-8">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">Track Process Journey</h3>
        <p className="text-gray-600 mb-4">
          Enter a Lead ID or PO ID to view the complete timeline from lead creation to invoice generation.
        </p>
      </div>
        ...
        <div className="mt-8">
          ...
          <div className="relative">
            ...
            <div className="space-y-8">
              ...
              <div className="relative flex items-start">
                ...
                <div className="ml-12 flex-1">
                  <div 
                    ...
                    onClick={() => timelineData.lead?._id && navigateToLead(timelineData.lead._id)}
                  >
                    ...
                    <div className="space-y-2 text-sm text-blue-800">
                      <p><strong>Customer:</strong> {timelineData.lead?.customerName || 'N/A'}</p>
                      <p><strong>Lead ID:</strong> {timelineData.lead?._id || 'N/A'}</p>
                      <p><strong>Created:</strong> {formatDate(timelineData.lead?.createdAt)}</p>
              ...
              <div className="relative flex items-start">
                <div className="absolute left-0 top-0">
                  {getStatusIcon(timelineData.quotation, !!timelineData.quotation)}
                </div>
                <div className="ml-12 flex-1">
                  <div 
                    ...
                    onClick={() => timelineData.quotation?._id && navigateToQuotation(timelineData.quotation._id)}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-lg font-semibold ${timelineData.quotation ? 'text-green-900' : 'text-gray-700'} flex items-center`}>
                        Quotation Issued
                      ...
                      <span className={`text-sm font-medium ${timelineData.quotation ? 'text-green-600' : 'text-gray-500'}`}>Step 2</span>
                    </div>
                    <div className={`space-y-2 text-sm ${timelineData.quotation ? 'text-green-800' : 'text-gray-600'}`}>
                      <p><strong>Quotation ID:</strong> {timelineData.quotation?._id || 'N/A'}</p>
                      <p><strong>Quotation Number:</strong> {timelineData.quotation?.quotationNumber || 'N/A'}</p>
                      <p><strong>Issued:</strong> {formatDate(timelineData.quotation?.createdAt)}</p>
                      <p><strong>Status:</strong> 
              ...
              <div className="relative flex items-start">
                ...
                <div className="ml-12 flex-1">
                  <div 
                    ...
                    onClick={() => timelineData.po?._id && navigateToPO(timelineData.po._id)}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-lg font-semibold ${timelineData.po ? 'text-purple-900' : 'text-gray-700'} flex items-center`}>
                        PO Generated
                        {timelineData.po?._id && (
                          <svg className="ml-2 w-4 h-4 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                          </svg>
                        )}
                      </h4>
                      <span className={`text-sm font-medium ${timelineData.po ? 'text-purple-600' : 'text-gray-500'}`}>Step 3</span>
                    </div>
                    <div className={`space-y-2 text-sm ${timelineData.po ? 'text-purple-800' : 'text-gray-600'}`}>
                      <p><strong>PO ID:</strong> {timelineData.po?._id || 'N/A'}</p>
                      <p><strong>PO Number:</strong> {timelineData.po?.poNumber || 'N/A'}</p>
                      <p><strong>Generated:</strong> {formatDate(timelineData.po?.createdAt)}</p>
                      <p><strong>Status:</strong> 
                        <span className={`ml-1 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          timelineData.po 
                          ...
                          {timelineData.po ? timelineData.po.status.toUpperCase() : 'NOT FOUND'}
                        </span>
                      </p>
                      {timelineData.po?._id && (
              ...
              <div className="relative flex items-start">
                ...
                <div className="ml-12 flex-1">
                  <div 
                    ...
                    onClick={() => timelineData.do1?._id && navigateToDO1(timelineData.do1._id)}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-lg font-semibold ${timelineData.do1 ? 'text-orange-900' : 'text-gray-700'} flex items-center`}>
                        DO1 Executed
                        {timelineData.do1?._id && (
                          <svg className="ml-2 w-4 h-4 text-orange-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                          </svg>
                        )}
                      </h4>
                      <span className={`text-sm font-medium ${timelineData.do1 ? 'text-orange-600' : 'text-gray-500'}`}>Step 4</span>
                    </div>
                    <div className={`space-y-2 text-sm ${timelineData.do1 ? 'text-orange-800' : 'text-gray-600'}`}>
                      <p><strong>DO1 ID:</strong> {timelineData.do1?._id || 'N/A'}</p>
                      <p><strong>DO1 Number:</strong> {timelineData.do1?.do1Number || 'N/A'}</p>
                      <p><strong>Executed:</strong> {formatDate(timelineData.do1?.createdAt)}</p>
                      <p><strong>Status:</strong> 
                        <span className={`ml-1 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          timelineData.do1 
                          ...
                          {timelineData.do1 ? timelineData.do1.status.toUpperCase() : 'NOT FOUND'}
                        </span>
                      </p>
                      {timelineData.do1?._id && (
              ...
              <div className="relative flex items-start">
                ...
                <div className="ml-12 flex-1">
                  <div 
                    ...
                    onClick={() => timelineData.do2?._id && navigateToDO2(timelineData.do2._id)}
                  >
                    <div className="flex items-center justify-between mb-2">
                      <h4 className={`text-lg font-semibold ${timelineData.do2 ? 'text-indigo-900' : 'text-gray-700'} flex items-center`}>
                        DO2 Executed → Invoice Generated
                        {timelineData.do2?._id && (
                          <svg className="ml-2 w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path>
                          </svg>
                        )}
                      </h4>
                      <span className={`text-sm font-medium ${timelineData.do2 ? 'text-indigo-600' : 'text-gray-500'}`}>Step 5</span>
                    </div>
                    <div className={`space-y-2 text-sm ${timelineData.do2 ? 'text-indigo-800' : 'text-gray-600'}`}>
                      <p><strong>DO2 ID:</strong> {timelineData.do2?._id || 'N/A'}</p>
                      <p><strong>DO2 Number:</strong> {timelineData.do2?.do2Number || 'N/A'}</p>
                      <p><strong>Executed:</strong> {formatDate(timelineData.do2?.createdAt)}</p>
                      <p><strong>Status:</strong> 
                        <span className={`ml-1 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          timelineData.do2 
                          ...
                          {timelineData.do2 ? timelineData.do2.status.toUpperCase() : 'NOT FOUND'}
                        </span>
                      </p>
                      {timelineData.invoice && (
                        <div className="mt-3 pt-3 border-t border-indigo-200">
                          <p><strong>Invoice Number:</strong> {timelineData.invoice.invoiceNumber}</p>
                          <p><strong>Invoice Generated:</strong> {formatDate(timelineData.invoice.generatedAt)}</p>
                          <p><strong>Tally Push:</strong> 
                            <span className={`ml-1 inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                              timelineData.invoice.pushedToTally 
                                ? 'bg-green-100 text-green-800' 
                                : 'bg-yellow-100 text-yellow-800'
                            }`}>
                              {timelineData.invoice.pushedToTally ? 'PUSHED' : 'PENDING'}
                            </span>
                          </p>
                        </div>
                      )}
      ...
      <div className="mt-8 p-4 bg-gray-50 border border-gray-200 rounded-md">
        ...
        <ul className="text-sm text-gray-700 space-y-1">
          <li>• Enter either a Lead ID or PO ID to view the complete timeline</li>
