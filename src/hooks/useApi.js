...
export const useApi = (config = {}) => {
  ...
  const getRetryDelay = (attempt) => {
    return finalConfig.retryDelay * Math.pow(finalConfig.retryDelayMultiplier, attempt);
  };
  ...
  const shouldRetry = (error, response, attempt) => {
    ...
    if (error && RETRY_ERROR_TYPES.some(type => error.message.includes(type))) {
      return true;
    }
   ...
   */
  const makeRequest = useCallback(async (url, options = {}, attempt = 0) => {
   ...
   */
  const retry = useCallback(async (url, options = {}) => {
    if (state.status === API_STATES.ERROR || state.status === API_STATES.TIMEOUT) {
